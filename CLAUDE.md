...
**Current state:** fresh project scaffold (default Nest starter files only — `app.module.ts`/`app.controller.ts`/`app.service.ts`). Nothing from the phased plan below is implemented yet; active phase is **Phase 0** in `docs/DEVELOPMENT_PHASES.md`. The `prisma/schema.prisma` schema is written but no migration has been run (no `generated/prisma` client, no `DATABASE_URL` wiring in code yet).
...
8. **All endpoints return the global response envelope** (`docs/CODING_STANDARDS.md` §Response Envelope) and follow the endpoint template in `docs/API_SPECIFICATION.md`.
...
npx prisma generate          # regenerate client to generated/prisma (schema.prisma output path)
```
...
- **No repository abstraction over Prisma** — Prisma client (generated to `generated/prisma`, gitignored) *is* the data layer; complex queries live as private methods on the owning service. Abstractions are added only when a second real implementation exists (e.g. `PaymentGatewayService` since Bosta is planned).
...
- **DB:** PostgreSQL via Prisma (`prisma/schema.prisma`, client output `generated/prisma`)
