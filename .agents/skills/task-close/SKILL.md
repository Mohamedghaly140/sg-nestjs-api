---
name: task-close
description: Checks a just-completed coding task against AGENTS.md's mandatory "After Completing Any Task" checklist (phase status, API spec, DATABASE.md, CHANGELOG, ADRs) before it's considered done.
user-invocable: false
---

# Task Close

Verifies the mandatory doc-update checklist in `AGENTS.md` was actually followed, instead of trusting memory.
...
Reproduced verbatim from `AGENTS.md` §"After Completing Any Task" — check each item against the actual diff (`git status` / `git diff --stat`), not against intent.
