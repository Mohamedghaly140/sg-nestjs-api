...
1. **Run the package-manager detector** (ships with ECC):
   ...
   It resolves the package manager (npm / pnpm / yarn / bun) from, in order: `CLAUDE_PACKAGE_MANAGER`, `.Codex/package-manager.json`, the `package.json` `packageManager` field, the lockfile, then global config.
...
.Codex/tdd/<plan-or-task-name>.tdd.md
```

If the repository already uses Codex-specific local artifacts, the `.Codex/tdd/` location is also acceptable. Include:
